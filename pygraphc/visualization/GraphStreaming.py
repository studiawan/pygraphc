...
class GraphStreaming:
    ...
    def change_color(self, gstream):
        ...
        for index, cluster in enumerate(self.clique_percolation):
            node_attributes = {'size': 10, 'r': cluster_color[index][0], 'g': cluster_color[index][1],
                               'b': cluster_color[index][2]}
            for node in cluster:
                gstream.change_node(node, **node_attributes)

    def gephi_streaming(self):
        gstream = pygraphc.pygephi.GephiClient('http://localhost:8080/workspace0', autoflush=True)
        gstream.clean()
